...
## Linear scan + CSR / Set Intersection + CSR
...
The fwd_bits + CSR approach is better for dense graphs with about 250,000 nodes and the Linear scan / Set intersection + CSR approach is better for sparse graphs with about 40 million nodes.

Time complexity wise, the Set intersection + CSR approach is better for sparse with about 40 million nodes and the fwd_bits + CSR approach is better for dense graphs with about 250,000 nodes.

All this is applicable on a system having a RAM of 8 GB.

But since most of the real-world graphs are sparse graphs, the Set intersection + CSR approach is the way to go. The fwd_bits + CSR approach can be used for very specific real-world applications where dense graphs rule, for example, protein-protein interaction networks, brain connectivity networks, co-authorship graphs in narrow academic fields, high frequency trading networks, etc. But all of these are small and dense, not large and dense! But even for large and dense graphs (1 million nodes => 10^12 edges), the storage requirements are in petabytes, which is nearly impossible to have.
